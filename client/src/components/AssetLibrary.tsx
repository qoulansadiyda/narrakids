...
export default function AssetLibrary({ category, onPick, onUpload, onPickAudio }: Props) {
  ...
  const isAudioCategory = category === "bgm" || category === "sfx";
          ...
          <div className="flex flex-col gap-2">
            {items.map((a) => (
              <div key={a.id} className="flex items-center gap-2 bg-white rounded-xl p-2 border-2 border-slate-100 shadow-sm hover:border-sky-200 transition-colors">
                ...
                <button
                  className={`text-[10px] px-3 py-1.5 rounded-lg border-b-2 font-black transition-colors shrink-0 uppercase tracking-wider active:translate-y-[2px] active:border-b-0 mb-[2px] ${category === "bgm" ? "bg-purple-400 hover:bg-purple-500 text-white border-purple-600 pb-2 mb-0" : "bg-blue-400 hover:bg-blue-500 text-white border-blue-600 pb-2 mb-0"}`}
                  onClick={() => {
                    onPickAudio?.(category as "bgm" | "sfx", a.id, a.src);
                  }}
                >
                  {category === "bgm" ? "+ Pilih BGM" : "+ Pilih SFX"}
                </button>
              </div>
            ))}
          </div>
